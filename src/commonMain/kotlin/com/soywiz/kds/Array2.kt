...
data class Array2<TGen>(val width: Int, val height: Int, val data: Array<TGen>) : Iterable<TGen> {
	companion object {
		...
		inline fun <reified TGen> withGen(width: Int, height: Int, gen: (x: Int, y: Int) -> TGen) = Array2(width, height, Array(width * height) { gen(it % width, it / width) })
		...
		inline operator fun <reified TGen> invoke(map: String, default: TGen, transform: Map<Char, TGen>): Array2<TGen> {
	...
	fun getPositionsWithValue(value: TGen) = data.indices.filter { data[it] == value }.map { Pair(it % width, it / width) }
