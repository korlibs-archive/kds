...
fun <K, V> MutableMapList<K, V>.appendAll(vararg items: Pair<K, V>): MutableMapList<K, V> = this.apply { for ((k, v) in items) append(k, v) }
fun <K, V> MutableMapList<K, V>.replaceAll(vararg items: Pair<K, V>): MutableMapList<K, V> = this.apply { for ((k, v) in items) replace(k, v) }
